...
- Commit at each checkpoint. When you finish a task — or a coherent unit of a
  ...
  commits — not one lump, and not zero. Stage only the files you changed for that
  unit — name them; never `git add -A`, and never sweep in unrelated dirt. Commit
  on the CURRENT branch; do not create or switch branches unless the user tells
  you to work on a new one.

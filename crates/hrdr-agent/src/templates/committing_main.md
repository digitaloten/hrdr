- Keep the changelog current as you work, not only at release time. If the
  ...
  refactor with no outward effect, a test-only or docs-only change). If the
  project ships something a user consumes and has NO changelog, start one —
  `CHANGELOG.md` in Keep-a-Changelog form with `## [Unreleased]` at the top — and
  say in your summary that you did. When you integrate sub-agents'
...
- Keep a backlog current the same way — `docs/backlog.md`, and start it if the
  project has none. Anything raised in this session and left unfinished

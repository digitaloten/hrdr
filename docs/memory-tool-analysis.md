...
Status: **implemented.** The whole target design below shipped:

- **§1 schema + §2 tool-maintained pointer index** (the keystone) — `670b1d0`:
  one memory = one `<slug>.md` with `name`/`description`/`type` frontmatter,
  `MEMORY.md` regenerated by the tool after every change (grouped by type),
  legacy frontmatter-less files read as `reference`.
- **§3 operations** — `670b1d0`: `view` / `write` / `edit` (field-in-place) /
  `delete` / `search`.
- **§5 auto-memory triggers** — `670b1d0`: baked into the tool description (save
  unprompted on "remember this" / a correction / a stated preference / a
  non-obvious decision; dedup before write; prune contradicted; absolute dates).
- **§4 relevance recall** — `47d3013`: per-turn, the pointer index loads at
  session start and a memory whose `description` matches the working context has
  its **body** injected into the model history (budget-bounded, both scopes).

44 unit tests cover the tool + recall. The design write-up is kept below as the
rationale. The remaining thread is **§G7 drift detection** (a periodic
prune/verify pass) — cheap now that §2 keeps files and index in sync, but not
yet built.
